      ...
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link " href="{{ url('/livreurs') }}">
            ...
            <span class="nav-link-text ms-1">Tableau des livreurs </span>
          </a>
        </li>
       

        <li class="nav-item">
          <a class="nav-link text-white text-white text-white active bg-gradient-primary" href="{{route('listcommendSL')}}">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
              <i class="material-icons opacity-10">format_list_numbered</i>
            </div>
            <span class="nav-link-text ms-1">Liste Commandes</span>
          </a>
        </li>
        ...
        <li class="nav-item">
          <a class="nav-link text-white " href="{{url('/profilesl')}}">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Tableau des commandes </li>
          </ol>
          ...
          <div class="card">
            <div class="card-header pb-0">
              <div class="row">
                <div class="col-lg-6 col-7">
                  <h6>Tableau des commandes</h6>
                  <p class="text-sm mb-0">
                    <i class="fa fa-check text-info" aria-hidden="true"></i>
                    <span class="font-weight-bold ms-1">Les commandes </span>se sont terminées 
                  </p>
                </div>
            ...
            <div class="card-body px-0 pb-2">
              <div class="table-responsive">
                ...
                <form action="affecterlivreur" method="POST">
                  <thead>
                    <tr>
                      ...
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">identifiant commende </th>
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Prix</th>
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Nom de la société </th>
                      ...
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">affecter</th>
                    </tr>
                  </thead>
                  <tbody>
                    
                  
                       @foreach($commadmins as $commadmin)
                       
                        @csrf
                        @if (Route::has('login'))

                        @auth
                        @if(Auth::user()->nomsociete==$commadmin->nom_soc)

                    <tr>
                       
                     <td> {{$commadmin->nom_user}} 
                    <input type="hidden" value="{{$commadmin->nom_user}}" name="nom_client"/>
                    </td>
                     <td>{{$commadmin->adresse_user}}
                    <input type="hidden" value="{{$commadmin->adresse_user}}" name="address_client"/>
                    </td>

                     <td align="center"> {{$commadmin->id}}</td>
                     <td align="center">{{$commadmin->prix_total}} TND 
                    <input type="hidden" value="{{$commadmin->prix_total}}" name="commande"/>
                    </td>
                     <td align="center">{{$commadmin->nom_soc}}</td>
                     <td align="center">
               
                      <SELECT name="nom_livreur" size="1">
                        @foreach($livreur as $liv)
                      <option value="{{$liv->nom}}">{{$liv->nom}}</option>
                        @endforeach
                      </SELECT>
                    ...
                    <a href="{{ url('invoice',$commadmin->id)}}">
                    <i class="material-icons opacity-10">
                     ...
                     <td align="center">
                      <button type="submit"  class="btn btn-link text-success px-3 mb-0"> affecter </button>
                      <i class="material-icons text-sm me-2"> how_to_reg</i>
                     </td>
                    </tr>
